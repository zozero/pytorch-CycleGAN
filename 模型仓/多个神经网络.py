...
def 定义生成器(输入的通道数, 输出的通道数, 末尾过滤器数量, 生成器网络名, 归一化类型='批', 是否使用失活率=False, 网络初始化类型='常规', 初始化比例因子=0.02, 图形处理单元标识码=[]):
    网络 = None
    层归一化 = 获得层归一化(归一化类型=归一化类型)

    if 生成器网络名 == '9块版残差神经网络':
        网络 = 残差神经网络生成器(输入的通道数, 输出的通道数, 末尾过滤器数量, 层归一化=层归一化, 是否使用失活率=是否使用失活率, 块数=9)
    elif 生成器网络名 == '6块版残差神经网络':
        网络 = 残差神经网络生成器(输入的通道数, 输出的通道数, 末尾过滤器数量, 层归一化=层归一化, 是否使用失活率=是否使用失活率, 块数=6)
    elif 生成器网络名 == 'U型网络_128':
        网络 = U型网络生成器(输入的通道数, 输出的通道数, 7, 末尾过滤器数量, 层归一化=层归一化, 是否使用失活率=是否使用失活率)
    elif 生成器网络名 == 'U型网络_256':
        网络 = U型网络生成器(输入的通道数, 输出的通道数, 8, 末尾过滤器数量, 层归一化=层归一化, 是否使用失活率=是否使用失活率)
    else:
...
class 残差神经网络生成器(nn.Module):
    def __init__(self, 输入的通道数, 输出的通道数, 末尾过滤器数量=64, 层归一化=nn.BatchNorm2d, 是否使用失活率=False, 块数=6, 填充类型='反射'):
        assert (块数 >= 0)
        ...
        模型 = [
            ...
            nn.Conv2d(输入的通道数, 末尾过滤器数量, kernel_size=7, padding=0, bias=是否使用偏差),
            层归一化(末尾过滤器数量),
            nn.ReLU(True)
        ]
        ...
        for i in range(下采样数量):
            倍数 = 2 ** i
            模型 += [
                nn.Conv2d(末尾过滤器数量 * 倍数, 末尾过滤器数量 * 倍数 * 2, kernel_size=3, stride=2, padding=1, bias=是否使用偏差),
                层归一化(末尾过滤器数量 * 倍数 * 2),
                nn.ReLU(True)
            ]
        ...
        倍数 = 2 ** 下采样数量
        for i in range(块数):
            模型 += [残差神经网络块(末尾过滤器数量 * 倍数, 填充类型=填充类型, 层归一化=层归一化, 是否使用失活率=是否使用失活率, 是否使用偏差=是否使用偏差)]
        ...
        for i in range(下采样数量):
            ...
            模型 += [
                ...
                层归一化(int(末尾过滤器数量 * 倍数 / 2)),
                nn.ReLU(True)
            ]
        模型 += [nn.ReflectionPad2d(3)]
        模型 += [nn.Conv2d(末尾过滤器数量, 输出的通道数, kernel_size=7, padding=0)]
        模型 += [nn.Tanh()]
...
class U型网络生成器(nn.Module):
    def __init__(self, 输入的通道数, 输出的通道数, 下采样数, 末尾过滤器数量=64, 层归一化=nn.BatchNorm2d, 是否使用失活率=False):
        super(U型网络生成器, self).__init__()
        U型网络块 = U型网络跳过连接块(末尾过滤器数量 * 8, 末尾过滤器数量 * 8, 输入的通道数=None, 子模块=None, 层归一化=层归一化, 是否为最内层模块=True)
        for i in range(下采样数 - 5):
            U型网络块 = U型网络跳过连接块(末尾过滤器数量 * 8, 末尾过滤器数量 * 8, 输入的通道数=None, 子模块=U型网络块, 层归一化=层归一化, 是否使用失活率=是否使用失活率)
        U型网络块 = U型网络跳过连接块(末尾过滤器数量 * 4, 末尾过滤器数量 * 8, 输入的通道数=None, 子模块=U型网络块, 层归一化=层归一化)
        U型网络块 = U型网络跳过连接块(末尾过滤器数量 * 2, 末尾过滤器数量 * 4, 输入的通道数=None, 子模块=U型网络块, 层归一化=层归一化)
        U型网络块 = U型网络跳过连接块(末尾过滤器数量, 末尾过滤器数量 * 2, 输入的通道数=None, 子模块=U型网络块, 层归一化=层归一化)
        self.模型 = U型网络跳过连接块(输出的通道数, 末尾过滤器数量, 输入的通道数=输入的通道数, 子模块=U型网络块, 是否为最外层模块=True, 层归一化=层归一化)
